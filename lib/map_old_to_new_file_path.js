...
/**
 * Replaces file names in the passed file_path with the file names in the file_mappings.
 *
 * @param file_path The path to the file where the filenames should be replaced.
 * @param file_mappings The file names to replace and the file names they should be replaced with.
 */
function map_old_to_new_file_path ( file_path, file_mappings ) {
	...
	for ( var old_path in file_mappings ) {
		...
		// Logs the mapping.
		if ( body !== new_body ) {
			grunt.log.writeln( "Old file path " + old_path + " found in patch. This path has been automatically replaced by " + new_path + "." );
		}
	}
